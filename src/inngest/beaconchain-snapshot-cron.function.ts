...
export const beaconchainSnapshotCron = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        // finally
        return { event, body: `Done at ${timestamp()}`, beaconchain: { epoch, queue, apr }, ms: { epochMs, queueMs, aprMs, xataMs, telegramMs } }
    },
)
